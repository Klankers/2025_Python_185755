"""Test program for CSE-41273 Homework 1.
    Put this in the same directory as your HW1.py file,
    then run from the command line (NOT in the REPL):
    python HW1_test.py

    NOTE that you can test individual exercises by giving the name of the
    test class in this format:
    python HW1_test.py -k <test_class_name>

    Where <test_class_name> is any one of:
    ComfortLevelTests
    DashStringifyTests
    FstringPracticeTests
    GetEarliestTests
    PerfectSquareTests
    SillyCaseTests
    StringInfoTests

    For example, to test only the silly_case function, run:
    python HW1_test.py -k SillyCaseTests
"""
...
class FstringPracticeTests(unittest.TestCase):
    ...
    def test_fstring_numbers1(self):
        ...
        numbers = [7, 12, 45, 32]
        numbers_saved = list(numbers)
        self.assertEqual(fstring_practice(numbers, 3), expected)
        self.assertEqual(numbers, numbers_saved, "Input list was modifed!")
